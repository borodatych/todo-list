...
class Arr
{
	...
	*/
	 ...
	 * @return mixed
	 */
	public static function get($array, $key, $default = NULL, $checkEmpty = FALSE)
	{
        ...
        {
			return ( isset($array[$key]) && ( !$checkEmpty || ($checkEmpty&&!empty($array[$key])) ) ) ? $array[$key] : $default;
		}
	}
	public static function cut(&$array, $key, $default = NULL, $checkEmpty = FALSE)
	{
		$data = static::get($array,$key,$default,$checkEmpty);
        if( gettype($array)=='object' ) unset($array->{$key}); else unset($array[$key]);
		return $data;
	}
    /**
     * Обязательно по ссылки, иначе на isset ругаться будет
    */
	public static function exist(&$value, $default = NULL)
    {
     ...
     * @param $cols     :на сколкьо колонок бить
     * @return array
     */
	public static function splitCol($arr,$cols)
    {
        $out = array();
        $count = count($arr);
        ...
        for( $c = 0; $c < $cols; $c++ )
        {
            $rows = ceil($count/($cols - $c));
            for( $r = 0; $r < $rows; $r++ ) $out[$r][] = $c==0 && $r==0 ? current($arr) : next($arr);
            $count -= $rows;
        }
